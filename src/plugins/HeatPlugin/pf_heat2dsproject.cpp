 ...
 * @param feemFile
 */
PF_Heat2DSProject::PF_Heat2DSProject(const FileName& feemFile)
    :PF_CommonFEMProject(Constants::FEEMHEAT2DSTATIC_MIMETYPE, feemFile)
{
    setRootProjectNode(PF_Heat2DSNodeTreeBuilder::buildTree(this));

    connect(this,&PF_Heat2DSProject::dataChanged,this,&PF_Heat2DSProject::updateData);
    /** 检测cad的变化 **/
    connect(m_document,&PF_Document::EntityChanged,this,&PF_Heat2DSProject::updateData);
}

PF_Heat2DSProject::~PF_Heat2DSProject()
{
    setRootProjectNode(nullptr);
    CleanUp();
}

void PF_Heat2DSProject::updateData()
{
    setRootProjectNode(PF_Heat2DSNodeTreeBuilder::buildTree(this));
}
 ...
 * @return bool
 */
bool PF_Heat2DSProject::Cuthill()
{
...
bool PF_Heat2DSProject::SortElements()
{
//    auto meshele = m_mesh->eles;
//    auto meshnode = m_mesh->nodes;
//    auto NumNodes = m_mesh->numNode;
...
bool PF_Heat2DSProject::WriteStatic2D(CBigLinProb &L)
{
...
bool PF_Heat2DSProject::StaticAxisymmetric(CBigLinProb &L)
{
    //    auto meshele = m_mesh->eles;
    //    auto meshnode = m_mesh->nodes;
    //    auto NumNodes = m_mesh->numNode;
    ...
    //                // contribution to be from Heatnetization in the block;
    //                t = labellist[El->lbl].HeatDir;
    //                if (labellist[El->lbl].HeatDirFctn != nullptr) // functional Heatnetization direction
    //                {
    //                    CString str;
    //                    CComplex X;
    //                    int top1, top2;
    ...
    //                    str.Format("r=%.17g\nz=%.17g\nx=r\ny=z\ntheta=%.17g\nR=%.17g\nreturn %s",
    //                               X.re, X.im, arg(X) * 180 / PI, abs(X), labellist[El->lbl].HeatDirFctn);
    //                    top1 = lua_gettop(lua);
    //                    if (lua_dostring(lua, str) != 0) {
    //                        //                        MsgBox("Lua error evaluating \"%s\"", labellist[El->lbl].HeatDirFctn);
    //                        exit(7);
    //                    }
    //                    top2 = lua_gettop(lua);
    //                    if (top2 != top1) {
...
void PF_Heat2DSProject::GetFillFactor(int lbl)
{
    //    auto meshele = m_mesh->eles;
    //    auto meshnode = m_mesh->nodes;
...
double PF_Heat2DSProject::ElmArea(int i)
{
...
void PF_Heat2DSProject::CleanUp()
{
    //    int k;
    ...
    //        for(k=0;k<NumBlockLabels;k++)
    //            if(labellist[k].HeatDirFctn!=nullptr) free(labellist[k].HeatDirFctn);
    //        free(labellist);
    //    }
 ...
 * @return QVariantMap
 */
QVariantMap PF_Heat2DSProject::toMap() const
{
//    qDebug()<<Q_FUNC_INFO;
    ...
    /** 保存关于projet的信息 **/
    QVariantMap pro;
    pro.insert("Project type","Heat2D");
 ...
 */
PF_Project::RestoreResult PF_Heat2DSProject::fromMap(const QVariantMap& map, QString* errorMessage)
{
 ...
 * @param node 要更新的节点
 */
static void createTree(PF_Heat2DSProject* pro,PF_HeatFEMNode* node)
{
    if(!pro) return;
    if(!node) return;
    ...
    comp_node->addNode(std::move(boundary_node));
 ...
 *
 * @param pro
 * @return std::unique_ptr<PF_HeatFEMNode>
 */
std::unique_ptr<PF_HeatFEMNode> PF_Heat2DSNodeTreeBuilder::buildTree(PF_Heat2DSProject *pro)
{
    ...
    auto  root = std::make_unique<PF_HeatFEMNode>(pro);
